...
public int[] copyEvens(int[] nums, int count) {
  ...
  int length=0;
  for(int i=0;i<nums.length && length<count;i++){
    if(nums[i]%2==0) result[length++]=nums[i];
  }
  return result;
}
...
public int[] copyEndy(int[] nums, int count) {
  ...
  int n=0;
  int len=0;
  for(int i=0;i<nums.length && len<count;i++){
    n=nums[i];
    if((n>=0&&n<=10) || (n>=90&&n<=100)) result[len++]=n;
  }
  return result;
}
...
public int scoreUp(String[] key, String[] answers) {
  ...
  for(int i=0;i<key.length;i++){
    if(key[i].equals(answers[i])) count+=4; //correct
    else if(!answers[i].equals("?")) count-=1; // incorrect & not blank
  }
  return count;
}
...
public String[] wordsWithout(String[] words, String target) {
  ...
  len=0;
  for(int i=0;i<words.length;i++) {
      if(!words[i].equals(target)) result[len++]=words[i];
  }
  return result;
}
...
int largestSpecialScore(int[] array){
  int result=0;
  for(int i=0;i<array.length;i++){
    if(array[i]%10==0 && array[i]>result) result=array[i];
  }
  return result;
}
...
public int sumHeights(int[] heights, int start, int end) {
  int changes=0;
  for(int i=start;i<end;i++){
    changes+=Math.abs(heights[i+1]-heights[i]);
  }
  return changes;
}
...
public int sumHeights2(int[] heights, int start, int end) {
  ...
  for(int i=start;i<end;i++){
    ...
    if(diff>0) changes+=2*(diff);
    else changes+=Math.abs(diff);
  }
  return changes;
}
...
public int bigHeights(int[] heights, int start, int end) {
  int bigSteps=0;
  for(int i=start;i<end;i++){
    if(Math.abs(heights[i+1]-heights[i])>=5) bigSteps++;
  }
  return bigSteps;
}
...
public String[] mergeTwo(String[] a, String[] b, int n) {
  ...
  int bInd=0;
  for(int i=0;i<n;i++){
    if(a[aInd].compareTo(b[bInd])<0) result[i]=a[aInd++];
    else if(a[aInd].compareTo(b[bInd])>0) result[i]=b[bInd++];
    else{
      result[i]=b[bInd++];
      aInd++;
    }
