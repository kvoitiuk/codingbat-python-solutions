...
public String frontBack(String str) {
  int n = str.length();
  if (n>1){
    return str.substring(n-1,n)+str.substring(1,n-1)+str.substring(0,1);
  }
  return str;
}
...
public boolean in3050(int a, int b) {
  return (Math.abs(a-35)<=5 && Math.abs(b-35)<=5) ||
         (Math.abs(a-45)<=5 && Math.abs(b-45)<=5);
}
